...
sub get_model_info()
{
  ...
  foreach my $model (@models)
  {
    ...
    if ($id_attr eq $opt_model_id)
    {
      ...
      # Obtain an aspect file for the aspect mode.
      my $aspect = '';

      # Obtain a common file for the common mode.
      my $common = '';
      ...
      foreach my $kind (@kinds)
      {
        if ($kind eq 'aspect' || $kind eq 'aspect-llvm')
        {
          ...
          my $generated_by = $aspect_tag->att('generated-by');

          if ($generated_by)
          {
            print_debug_debug("Model will be generated by script '$generated_by'");

            if ($generated_by eq 'rerouter')
            {
