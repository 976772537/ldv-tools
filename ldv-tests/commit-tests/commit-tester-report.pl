...
if($mode == 2)
{
	create_double_report($results1, $results2);
}
else
{
	create_report($results1);
}
...
sub get_opt()
{
	...
	die "You should set --results" unless($results1);
	die "Couldn't find file '$results1'!" unless(-f $results1);
	$results1 = abs_path($results1);
	if($results2)
	{
		die "Couldn't find file '$results2'!" unless(-f $results2);
		$results2 = abs_path($results2);
		$mode = 2;
	}
