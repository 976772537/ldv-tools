			...
			const struct usb_device_id *id)
{
	...
	ASSURE_SUCCESS(result = class_register(dummy_class));
	ASSURE_SUCCESS(result = register_chrdev_region(*dummy_dev, DUMMY_COUNT, dummy_name));
	ASSURE_SUCCESS(result = usb_gadget_probe_driver(dummy_driver, dummy_bind));
	result = usb_gadget_unregister_driver(dummy_driver);
	class_destroy(dummy_class);
	ASSURE_ERROR(result = usb_gadget_probe_driver(dummy_driver, dummy_bind));
