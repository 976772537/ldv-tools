...
EM.run do
  agent = Nanite::Agent.new(:host => 'localhost', :user => 'mapper', :pass => 'testing', :vhost => '/nanite', :log_level => 'debug', :format => :json, :ping_time => 4, :identity => 'tester', :initrb => 'tester.rb')
	...
	(1..1).each do |i|
		Nanite.push("/ldvqueue/queue", { :type => 'ldv', :args => "task #{i}", :workdir=>'/home/pavel/work/ldv/test', :key => "ldv.#{i}", :env => {'RCV_TIMEOUT'=>20,'LDV_DEBUG'=>30,'envs'=>'linux-2.6.31.6.tar.bz2','drivers'=>'wl12xx.ko.tar.bz2','rule_models'=>'32_7'} }, :offline_failsafe => true, :selector => :ldv_selector)
