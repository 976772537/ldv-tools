...
EM.run do
  agent = Nanite::Agent.new({:host => 'localhost', :log_level => 'debug', :format => :json, :ping_time => 4, :identity => 'tester', :initrb => 'tester.rb'}.merge opts_p.options)
	...
	(1..1).each do |i|
		...
		Nanite.push("/ldvqueue/queue", { :type => 'ldv', :args => "task #{i}", :workdir=>'/tmp/cluster/wd', :key => "ldv.#{i}", :env => {'RCV_TIMEOUT'=>20,'envs'=>'linux-2.6.31.6.tar.bz2','drivers'=>'wl12xx.ko.tar.bz2','rule_models'=>'32_7'}, 'parent_machine' => {:host=>'127.0.0.1',:sshuser=>'pavel',:root=>'/tmp/cluster/wd'} }, :offline_failsafe => true, :selector => :ldv_selector)
